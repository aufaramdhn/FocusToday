                ...
                @if($heroArticle)
                    <div>
                        <a href="{{ route('articles.show', $heroArticle->slug) }}">
                            <img src="{{ $heroArticle->thumbnail_url }}" alt="{{ $heroArticle->title }}" class="w-full h-[620px] object-cover rounded hover:opacity-90 transition">
                        </a>
                        <div class="mt-4">
                            <a href="{{ route('articles.show', $heroArticle->slug) }}">
                        ...
                        <div class="hidden lg:flex flex-col gap-3">
                            @foreach($latestArticles->take(3) as $article)
                                <div class="flex gap-3">
                                    <img src="{{ $article->thumbnail_url }}" alt="{{ $article->title }}" class="h-[140px] w-[240px] object-cover rounded flex-shrink-0">
                                    <div>
                    ...
                    @if($readMoreHero = $latestArticles->skip(3)->first())
                        <div class="mb-2">
                            <a href="{{ route('articles.show', $readMoreHero->slug) }}">
                                <img src="{{ $readMoreHero->thumbnail_url }}" alt="{{ $readMoreHero->title }}" class="h-[270px] w-full object-cover rounded hover:opacity-90 transition">
                            </a>
                            <div class="mt-2">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($sidebarCategories as $category)
                            ...
                            <div class="flex flex-col gap-2">
                                <a href="{{ route('articles.show', $article->slug) }}">
                                    <img src="{{ $article->thumbnail_url }}" alt="{{ $article->title }}" class="h-[140px] w-full object-cover rounded hover:opacity-90 transition">
                                </a>
                                <div class="">
                ...
                <div>
                    ...
                    @if($mainVideo)
                        @php
                            ...
                            $thumb   = data_get($mainVideo, 'snippet.thumbnails.maxres.url') 
                                    ?? data_get($mainVideo, 'snippet.thumbnails.high.url') 
                                    ?? data_get($mainVideo, 'snippet.thumbnails.medium.url');
                        @endphp
                        <a href="https://www.youtube.com/watch?v={{ $videoId }}" target="_blank" class="group block">
                            <div class="relative">
                    ...
                    <div class="flex flex-col gap-3 mt-4">
                        @foreach($sideVideos as $video)
                            ...
                            <a href="https://www.youtube.com/watch?v={{ $sideId }}" target="_blank" class="flex gap-3 group">
                                ...
                                <div class="flex flex-col justify-center">
                                    <h4 class="font-semibold text-sm line-clamp-2 leading-snug group-hover:text-blue-600 transition">
                                        {!! $sideTitle !!}
                                    </h4>
