        ...
        <div x-data="{
                ...
                <div class="flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex flex-row-reverse items-center gap-2 md:w-[300px] w-full">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Search tags..." class="w-full focus:outline-none rounded-md px-3">
                    ...
                    <div class="flex items-center gap-4 flex-wrap">
                        ...
                        @if (request()->hasAny(['search', 'start_date', 'end_date', 'sort']))
                            <a href="{{ route('admin.tag.index') }}" class="text-red-500 text-sm hover:underline">
                                Reset Filter
                            </a>
                        @endif
                    </div>
            ...
            <div
                ...
                <div class="">
                    <p class="">
                        Showing
                        <span class="font-bold">{{ $tags->firstItem() }}</span>
                        to
                        <span class="font-bold">{{ $tags->lastItem() }}</span>
                        of
                        <span class="font-bold">{{ $tags->total() }}</span>
                        results
                    </p>
                </div>
                <div class="">
                    @if ($tags->hasPages())
                        <nav>
                            <ul class="inline-flex flex-wrap items-center gap-2">
                                ...
                                @foreach (range(1, $tags->lastPage()) as $page)
                                    <li>
