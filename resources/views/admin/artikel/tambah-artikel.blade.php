<x-layout-admin>
    ...
    <div class="">
        ...
        <x-breadcrumb :items="[
            ...
            ['label' => 'Add Article', 'url' => '/dashboard/artikel/tambah'],
        ]" />
        ...
        <div class="bg-white rounded-lg shadow-md w-full p-6 mt-6">
            <form class="flex flex-col gap-4" method="POST" action="{{ route('admin.artikel.store') }}"
                ...
                <div class="flex flex-col gap-2" x-data="thumbnailPreview('{{ isset($article) && $article->thumbnail ? asset('storage/' . $article->thumbnail) : '' }}')">
        ...
        <script>
            function thumbnailPreview(existingUrl = '') {
                return {
            ...
            function articleBlocks() {
                return {
                    ...
                    initEditor(index) {
                        this.$nextTick(() => {
                            const selector = `#editor-${index}`;
                            if (document.querySelector(selector).classList.contains('ql-container')) return;
